...
const SpecialtySection: React.FC<SpecialtySectionProps> = ({ specialty, index = 0 }) => {
  ...
  const handleInfoClick = useCallback(() => {
    setContactPrefill({
      subject: `${specialty.title} Hakkında Bilgi`,
      message: `${specialty.title} hakkında detaylı bilgi almak istiyorum.`,
    });
    router.push('/contact');
  ...
  return (
    <div className="group w-full bg-[#1a1a1a] border border-gray-800 rounded-2xl overflow-hidden hover:border-blue-primary/30 transition-all duration-500 shadow-lg hover:shadow-blue-primary/10 hover:-translate-y-1">
      <div className={`flex flex-col md:flex-row ${isReversed ? 'md:flex-row-reverse' : ''} h-full`}>
        {/* Image Section */}
        <div className="w-full md:w-2/5 relative min-h-[240px] md:min-h-full overflow-hidden shrink-0">
          <Image
            src={imageSrc}
            alt={specialty.title}
            fill
        ...
        <div className="w-full md:w-3/5 p-8 md:p-10 flex flex-col justify-center relative z-10">
          <div className="flex flex-col gap-4">
            <div>
              <h3 className="text-2xl md:text-3xl font-bold text-white mb-2 group-hover:text-blue-primary transition-colors">{specialty.title}</h3>
              <div className="w-12 h-1 bg-blue-primary rounded-full mb-4"></div>
              <p className="text-gray-400 text-sm md:text-base leading-relaxed">{specialty.description}</p>
            </div>
